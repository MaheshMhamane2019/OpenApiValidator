...
namespace Conscience.OpenApiValidator
{
	...
	///
	/// </summary>
	public class OpenApiValidatorBuilder : IOpenApiValidatorBuilder
	{
		private readonly IOpenApiRuleFactory _ruleFactoryRule;

		public OpenApiValidatorBuilder(IValidatorContext validatorContext) => _ruleFactoryRule = new OpenApiRuleFactory(validatorContext);

		public IEnumerable<IValidate> Build(OpenApiDocument doc) => this.WalkAndBuild(doc);

		/// <summary>
		///
		/// </summary>
		/// <param name="doc">OpenApiDocument to be walked</param>
		internal IEnumerable<IValidate> WalkAndBuild(OpenApiDocument doc)
		{
			if (doc != null && doc.Paths != null)
				foreach (var pathItem in doc.Paths)
				{
					var pathParameters = pathItem.Value.Parameters ?? new List<OpenApiParameter>();
					foreach (var item in WalkAndBuild(pathItem, pathParameters))
					{
						yield return item;
					}
				}
		...
		///
		/// </summary>
		internal IEnumerable<IValidate> WalkAndBuild(KeyValuePair<string, OpenApiPathItem> pathItem, IEnumerable<OpenApiParameter> openApi_HttpPathParameters)
		{
			if ((pathItem.Key != null && pathItem.Key.Length > 0) || pathItem.Value != null)
			{
				foreach (var operation in pathItem.Value.Operations)
				{
					var httpRequestValidator = _ruleFactoryRule.Build(pathItem.Key, operation);
					var openApi_HttpParameters = operation.Value.Parameters.Concat(openApi_HttpPathParameters);

					httpRequestValidator.AddRule(WalkAndBuild(openApi_HttpParameters));
					httpRequestValidator.AddRule(WalkAndBuild(operation.Value.RequestBody));

					//WalkAndBuild(OpenApiConstants.Components, () => WalkAndBuild(doc.Components));
					//WalkAndBuild(OpenApiConstants.Security, () => WalkAndBuild(doc.SecurityRequirements));
					//WalkAndBuild(OpenApiConstants.ExternalDocs, () => WalkAndBuild(doc.ExternalDocs));
					//WalkAndBuild(doc as IOpenApiExtensible);

					yield return httpRequestValidator;
				}
			}
			yield break;
		}
		...
		///
		/// </summary>
		internal IEnumerable<IValidate> WalkAndBuild(IEnumerable<OpenApiParameter> openApi_HttpParameters)
		{
			if (openApi_HttpParameters != null)
			{
				foreach (var openApi_HttpParameter in openApi_HttpParameters)//.Where(e => e.Deprecated.IsFalse()).AsEnumerable())
				{
					if (openApi_HttpParameter != null)
					{
						var rule = _ruleFactoryRule.Build(openApi_HttpParameter);

						if (openApi_HttpParameter.Schema != null)
							rule.AddRule(WalkAndBuild(openApi_HttpParameter.Schema));

						if (openApi_HttpParameter.Content != null && openApi_HttpParameter.Content.Count > 0)
							rule.AddRule(WalkAndBuild(openApi_HttpParameter.Content));
		...
		///
		/// </summary>
		internal IEnumerable<IValidate> WalkAndBuild(OpenApiRequestBody openApi_HttpRequestBody)
		{
			if (openApi_HttpRequestBody != null)
			{
				var requestBodyValidator = _ruleFactoryRule.Build(openApi_HttpRequestBody);

				requestBodyValidator.AddRule(WalkAndBuild(openApi_HttpRequestBody.Content));

				//WalkAndBuild(requestBody as IOpenApiExtensible);
				yield return requestBodyValidator;
			}
			yield break;
		}
		...
		///
		/// </summary>
		internal IEnumerable<IValidate> WalkAndBuild(IDictionary<string, OpenApiMediaType> openApi_HttpMediaTypes)
		{
			if (openApi_HttpMediaTypes != null)
			{
				foreach (var openApi_HttpMediaType in openApi_HttpMediaTypes)
					if (openApi_HttpMediaType.Key != null && openApi_HttpMediaType.Value != null)
					{
						var rule = _ruleFactoryRule.Build(openApi_HttpMediaType);

						//rule.AddRule(WalkAndBuild(openApi_HttpMediaType.Value.Schema));
						//rule.AddRule(WalkAndBuild(openApi_HttpMediaType.Value.Encoding));
						//WalkAndBuild(OpenApiConstants.Example, () => WalkAndBuild(mediaType.Examples));
						//WalkAndBuild(mediaType as IOpenApiExtensible);
						yield return rule;
					}

				//foreach (var content in WalkAndBuild(mediaType))
				//	{
				//		yield return content;
				//	}

				//var rule = _ruleFactoryRule.Build(content);
				//foreach (var mediaType in content)
				//{
				//	rule.AddRule(WalkAndBuild(mediaType));
				//}
				//yield return rule;
			}
		...
		///
		/// </summary>
		//internal IEnumerable<IValidate> WalkAndBuild(KeyValuePair<string, OpenApiMediaType> openApi_HttpMediaType)
		//{
		//	if (openApi_HttpMediaType.Key.IsNotNullOrEmpty() && openApi_HttpMediaType.Value != null)
		//	{
		//		var rule = _ruleFactoryRule.Build(openApi_HttpMediaType);
		...
		///
		/// </summary>
		internal IEnumerable<IValidate> WalkAndBuild(IDictionary<string, OpenApiEncoding> openApi_HttpEncodings)
		{
			if (openApi_HttpEncodings != null)
			{
				var rule = _ruleFactoryRule.Build(openApi_HttpEncodings);

				foreach (var encoding in openApi_HttpEncodings)
					rule.AddRule(WalkAndBuild(encoding));
		...
		///
		/// </summary>
		internal IEnumerable<IValidate> WalkAndBuild(KeyValuePair<string, OpenApiEncoding> openApi_HttpEncoding)
		{
			if (openApi_HttpEncoding.Key != null && openApi_HttpEncoding.Value != null)
			{
				var rule = _ruleFactoryRule.Build(openApi_HttpEncoding).FirstOrDefault();
				//WalkAndBuild(encoding as IOpenApiExtensible);
				yield return rule;
			}
		...
		///
		/// </summary>
		internal IValidate WalkAndBuild(OpenApiSchema openApi_HttpSchema)
		{
			return _ruleFactoryRule.Build(openApi_HttpSchema);
		}		
	}
