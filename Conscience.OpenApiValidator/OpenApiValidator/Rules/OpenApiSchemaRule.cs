...
namespace Conscience.OpenApiValidator
{
	...
	/// </summary>	 
	[Serializable]
	public class OpenApiSchemaRule : ValidatorRule<string, OpenApiSchema>
	{
		...
		///
		/// </summary>
		public OpenApiSchemaRule(IValidatorContext context, OpenApiSchema element) : base(context, element)
		{
		...
		public virtual RuleLogs Validate()
		{
			...
			catch
			{
				try
				{
					var jsonv = JsonValue.Parse(Data);
					//JsonSerializer serializer = new JsonSerializer();
					//var jsonValue = serializer.Serialize(Data);
					var valid = _jsonSchema.Validate(jsonv);
